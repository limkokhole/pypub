...
def save_image(image_url, image_directory, image_name):
    ...
    try:
        ...
        with open(full_image_file_name, 'wb') as f:
            ...
            requests_object = requests.get(image_url, headers=request_headers, timeout=30)
            try:
...
class ChapterFactory(object):
    ...
    def create_chapter_from_url(self, url, title=None):
        ...
        """
        try:
            request_object = requests.get(url, headers=self.request_headers, allow_redirects=False, timeout=30)
